...
coffeeProfile=function(id,name,base,weights){
  return {
    id,
    name,
    weights,
    requirements:coffeeDifficultyValues(base,weights,{min:1.15,max:9.2,low:.72,spread:.28})
  };
};
...
coffeeRivalProfile=function(r){
  ...
  return coffeeProfile(`rival-${r.id}`,r.name,base,weights);
};
...
coffeeEventProfile=function(type){
  ...
  return coffeeProfile(`event-${type}`,coffeeEventNames[type],base,coffeeEventWeights[type]);
};
